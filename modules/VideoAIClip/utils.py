...
# 3、基于图片差异阈值【超参】，提取关键帧，并导出时间戳
def extract_keyframes_with_timestamps(input_file, output_pattern, diff=0.01):
    command = [
        "ffmpeg",
        "-i", input_file,                                          # 指定输入
...
def split_frames_into_intervals(input_file, timestamps, temp_dir, output_base_dir, fps):
    ...
    temp_dir_len = int(len(os.listdir(temp_dir))*0.9)  # 【超参：各关键帧提取的比例】
    #frame_files = [f for f in sorted(os.listdir(temp_dir)) if f.startswith('frame_') and f.endswith('.jpg')]
    
    frame_files = [f for f in sorted(os.listdir(temp_dir)[:temp_dir_len]) if f.startswith('frame_') and f.endswith('.jpg')]
    ...
    for i in range(len(timestamps)):
        ...
        for frame_file in frame_files:
            ...
            print(f"Frame ID: {frame_index}, Frame time: {frame_time}, Interval: [{start_time}, {end_time})")
...
# 10、【优化】图片超参："差异度" > 30000时，再进行解析
def extract_text_from_images_with_difference(directory, threshold=30000):
    combined_text = ""
    image_files = [f for f in sorted(os.listdir(directory))]
    tmp_image_files = image_files.copy()

    previous_image = None
    for image_file in tmp_image_files:
        image_path = os.path.join(directory, image_file)
        
        if image_file == image_files[0]:  # 默认解析第一张图片【关键帧】
            print(image_file)
            ...
            if difference > threshold:
                print('差异度为：%d'%difference)
                print('带解析的关键帧-文件名%s'%image_file)   # 打印解析图片名
                text = image_to_text(image_path)
                combined_text += text 
                previous_image = image_path
        
                # 为了防止无限套娃，所以加上一个image_files每解析一次，就掐去“头部”一次
                new_head = int(image_file[6:-4])  
                tmp_image_files = image_files[new_head:]
...
def AI_Video_Clip(video_path,output_file):
    ...
    print('关键帧提取完成，个数为：%s'%len(timestamps))
    ...
    for combined_text,time in zip(interval_combined_text_list,interval_combined_video_time_list):
        ...
        #prompt = '将上述内容转换成中文视频解说词，要求语言精炼、有逻辑、以第三人称输出解说词部分、不要加入"我们"的元素，仅描述一段内容即可，解说词的内容需要远小于%s内，注意根据其时长控制字数'%time
        #prompt_1 = '将上述内容翻译成中文视频解说词，要求：1、背景为篮球场；2、语言风格为紧张、有逻辑、以第三人称；3、格式仅输出解说词部分、不要加入"我们"、"画面"等类似的描述，注意根据其时长（%s）控制字数'%time
        prompt_1 =  '将上述内容翻译成中文视频解说词，要求：1、仅输出解说词部分;2、时间在倒计时，一个人运球突破然后扣篮，然后一个人的脸在灯光的照射下逐渐被覆盖，最终显示他的名字；3、不要出现篮球以外的角色；4、不要出现几点几分；5、不要出现“长颈鹿”；6、不要有太多修饰'
        Question_1 = combined_text + prompt_1
        commentary = text_to_commentary(Question_1).replace('\n','')
        print('每张图片的解析内容：%s'%commentary)
        
        print('原内容：%s'%combined_text)
        commentary_list.append(commentary)
        
        summary = summary + combined_text
        
    prompt_2 = '将上述内容翻译成中文视频解说词，要求：1、仅输出解说词部分;2、时间在倒计时，一个人运球突破然后扣篮，最终一个人的脸在灯光的照射下逐渐被覆盖，最终显示出他的名字；3、不要出现篮球以外的角色；4、不要出现几点几分；5、不要出现“长颈鹿”'
    Question_2 = summary + prompt_2
    Summary = text_to_commentary(Question_2).replace('\n','')
    print('视频摘要为：%s'%Summary)
    ...
    for id_,commentary in enumerate(commentary_list):
        ...
        text_to_audio(commentary, audio_folder)


    # 5、合并所有关键帧下的视频
    video_name_list = sorted(os.listdir(os.path.join(os.getcwd(), 'Video', 'key_frame_video')),reverse=True)
    audio_name_list = sorted(os.listdir(os.path.join(os.getcwd(), 'Video', 'commentary_audio')),reverse=True)
